...
function generateFinanceExportFileFromRegistry(payload) {
  try {
    ...
    // Embed Xeenaps Logo (Base64) - Replacing text logo as requested
    const logoFile = DriveApp.getFileById('1ZpVAXWGLDP2C42Fct0bisloaQLf2095_');
    const logoBlob = logoFile.getBlob();
    const logoBase64 = Utilities.base64Encode(logoBlob.getBytes());
    const logoHtml = '<img src="data:' + logoBlob.getContentType() + ';base64,' + logoBase64 + '" style="height:35px; width:auto;" />';
