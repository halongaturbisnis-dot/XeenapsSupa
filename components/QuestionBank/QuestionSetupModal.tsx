...
const QuestionSetupModal: React.FC<QuestionSetupModalProps> = ({ item, onClose, onComplete }) => {
  ...
  // HYBRID LOGIC: Determine if AI mode is available
  const canUseAI = !selectedSource || !!selectedSource.extractedJsonId;

  // AUTO-SWITCH to MANUAL if invalid source selected
  useEffect(() => {
    if (selectedSource && !selectedSource.extractedJsonId) {
       setMode('MANUAL');
    }
  }, [selectedSource]);
  ...
  useEffect(() => {
    ...
    const delayDebounceFn = setTimeout(async () => {
      ...
      try {
        // Hybrid Fetch: 'research' path fetches ALL items (with or without JSON)
      ...
      <div className="bg-white rounded-[2rem] md:rounded-[3rem] w-full max-w-4xl shadow-2xl overflow-hidden relative flex flex-col max-h-[95vh] md:max-h-[90vh]">
        ...
        <div className="px-6 md:px-8 py-6 border-b border-gray-100 flex items-center justify-between shrink-0">
          ...
          <div className="flex bg-gray-100 p-1 rounded-xl">
             <button 
               ...
               disabled={!canUseAI}
               className={`px-4 py-2 rounded-lg text-[9px] font-black uppercase transition-all flex items-center gap-2 ${mode === 'AI' ? 'bg-[#004A74] text-white shadow-md' : (!canUseAI ? 'text-gray-300 cursor-not-allowed opacity-50' : 'text-gray-400')}`}
               title={!canUseAI ? "Requires extracted content" : "AI Generation"}
             >
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto custom-scrollbar p-6 md:p-8 space-y-8">
            ...
            {!item ? (
              <div className="relative" ref={searchContainerRef}>
                    ...
                    <div className="max-h-60 overflow-y-auto custom-scrollbar">
                      {isSearchingSources ? (
                        ...
                        availableSources.map(src => (
                          ...
                          >
                            <BookOpenIcon className={`w-4 h-4 mt-0.5 group-hover:text-[#004A74] ${!src.extractedJsonId ? 'text-orange-300' : 'text-gray-300'}`} />
                            <div className="min-w-0">
                               ...
                               <p className="text-[8px] font-bold text-gray-400 uppercase tracking-tighter mt-0.5">
                                 {src.authors[0]} • {src.year} • {src.topic}
                                 {!src.extractedJsonId && <span className="ml-2 text-orange-400">(Manual Only)</span>}
                               </p>
                            </div>
                          </button>
                        ))
               ...
               <FormField label="Context Source">
                 <div className="w-full px-5 py-3 bg-[#004A74]/5 border border-[#004A74]/10 rounded-xl text-[10px] font-black text-[#004A74] uppercase truncate flex items-center justify-between">
                    <span>{item.title}</span>
                    {!item.extractedJsonId && <span className="text-[8px] bg-orange-100 text-orange-600 px-2 py-0.5 rounded-full">Manual Only</span>}
                 </div>
               </FormField>
            )}
          ...
          <div className="pt-8 flex justify-end">
            <button type="submit" disabled={isGenerating} className="w-full md:w-auto px-12 py-5 bg-[#004A74] text-[#FED400] rounded-2xl font-black uppercase tracking-[0.2em] text-[10px] shadow-2xl hover:scale-105 transition-all flex items-center justify-center gap-4">
